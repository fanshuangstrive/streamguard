...
| | 逐请求基础信息 | 每个请求一行：`方法 路径 → 状态码 · 耗时`，命中限流排队时附「排队」标记（默认显示，可关） |
...
- **逐请求基础信息**：每个代理请求写入一行基础信息（方法/路径/状态码/耗时，命中限流排队时附「排队」标记），
  由 `internal/app` 的请求钩子上报，**仅含基础元信息，不含请求体与响应体**（符合开发规范第 9 节：详细日志不入面板）。
...
- [x] 逐请求基础信息显示（方法/路径/状态码/耗时/排队标记）+「显示请求日志」开关（后端钩子上报，无新配置项）
