...
| | 逐请求基础信息 | 每个请求一行：`方法 路径 → 状态码 · 耗时`；chat 请求附模型名与请求体大小（`· gpt-4o-mini · 12.3KB`），命中限流排队时附「排队」标记（默认显示，可关） |
...
- **逐请求基础信息**：每个代理请求写入一行基础信息（方法/路径/状态码/耗时，命中限流排队时附「排队」标记），
  chat 路径（`/chat/completions`）额外提取**模型名**与**请求体字节数**（`formatBytes` 转 KB/MB）展示。
  由 `internal/app` 的请求钩子上报，**仅含基础元信息（模型名属请求体顶层字段，不含消息内容），不含请求体与响应体正文**（符合开发规范第 9 节：详细日志不入面板）。
...
| 项 | 选型 | 说明 |
|---|---|---|
| 框架 | 原生 HTML/TS/CSS | 零框架；TS 模块拆分 main.ts / theme.ts / toast.ts，vite 构建 |
...
- [x] 逐请求基础信息显示（方法/路径/状态码/耗时/排队标记，chat 请求附模型名+请求体大小）+「显示请求日志」开关（后端钩子上报，无新配置项）
